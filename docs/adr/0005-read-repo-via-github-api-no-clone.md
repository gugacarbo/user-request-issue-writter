...
- **Footprint:** zero custo de disk/git no VPS; imagem Docker pode omitir git.
- **Credenciais mínimas:** apenas o PAT (sem SSH keys/deploy keys).
- **Eficiência:** leitura sob demanda (só os arquivos que o LLM pedir).
- **Segurança:** tools confinadas ao `owner/repo` do webhook.

## Opções consideradas

### Opção 1 — Leitura via GitHub REST API com PAT (escolhida)
**Prós:**
- Zero custo de disk/git no VPS (sem `.git`, sem working tree, sem `git` no
  container); imagem Docker pode omitir git.
- Funciona com repositórios privados usando apenas o PAT (sem SSH keys ou
  deploy keys adicionais).
- Leitura sob demanda (só os arquivos que o LLM pedir) reduz I/O vs.
  clonar tudo.
**Contras:**
- Rate limits da GitHub API (5000 req/h autenticado); para volume baixo é
  confortável, mas se crescer precisa cache de árvore/conteúdo.
- Arquivos muito grandes custam uma request de API para depois serem
  truncados; mitigado com checagem de `size` antes de baixar.

### Opção 2 — Shallow clone (`git clone --depth 1`)
**Prós:** leitura local rápida.
**Contras:** exige `git` no container, disk temporário por requisição, e
credenciais de clone (SSH/deploy key) além do PAT — infraestrutura e
superfície maiores.

### Opção 3 — Download de tarball do repo
**Prós:** um fetch só.
**Contras:** baixa o repo inteiro mesmo que o LLM precise de poucos arquivos
— desperdício de banda/disk.
...
## Confirmação
```bash
grep -q 'git' Dockerfile && echo "FALHA: git no container" || echo "ok: sem git"
grep -n "getRepoTree\|getFileContent\|getRepoInfo" src/github.ts
```

## Notas
- As tools de leitura (`list_files`, `read_file`, `get_repo_info`) são
  definidas no ADR-0004 e operam exclusivamente no `owner/repo` do webhook.